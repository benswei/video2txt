...
[![Tauri App](https://img.shields.io/badge/Tauri-v2-blue.svg)](https://tauri.app/)
...
<p align="center">
  <img src="src-tauri/icons/icon.png" alt="Video2Txt Logo" width="120" />
</p>
...
## ⚙️ 接入各类 AI Agent (如 Claude Code, OpenClaw 等)

### 直接调用
任何 AI Agent 均可运行 `run.sh` 脚本进行一键调用：

```bash
bash /path/to/video2txt/run.sh --input "/path/to/video.mp4"
```

### Claude Code 技能接入
可将此目录作为 Skill 放入 `~/.shared-skills/`，之后在 Claude 终端中可直接输入：
> "帮我转录一下这个视频，并生成摘要"
> "把这个 B站 链接的内容整理出来"
